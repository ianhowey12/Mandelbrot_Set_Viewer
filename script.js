...
class Display {
    ...
    compileProgram() {
        ...
        Display.getRenderSource(
                this.iterations, this.zR, this.zI, this.gradient, this.period, this.numColors, this.colorsInt, this.renderInverse
        ));
    ...
    /**
     * Converts a point in screen space [0, width] x [0, height] to the complex plane
     *
     * @param {number} _x
     * @param {number} _y
    ...
    screenToSpace(_x, _y) {
        ...
        o.r = (((2 * x) / unit) - 2) / this.scaling + this.offsetX;
        o.i = (-((2 * y) / unit) + 1.1) / this.scaling - this.offsetY;
    ...
    scaleAround(scalingFactor, _centerX, _centerY) {
        ...
        this.offsetX = ((2 * centerX) / unit - 2) * ((1 / this.scaling) - (1 / newScaling)) + this.offsetX;
        this.offsetY = -(-(2 * centerY) / (unit) + 1.1) * ((1 / newScaling) - (1 / this.scaling)) + this.offsetY;
    ...
    */
    processMultiTouchGesture(oldCoords, newCoords) {
        ...
        newCenter.x *= (0.5);
        newCenter.y *= (0.5);
        const oldLen = (((oldCoords[0].x - oldCoords[1].x) ** 2) + ((oldCoords[0].y - oldCoords[1].y) ** 2)) ** 0.5;
        const newLen = (((newCoords[0].x - newCoords[1].x) ** 2) + ((newCoords[0].y - newCoords[1].y) ** 2)) ** 0.5;
    ...
    */
    handleMouseMove(event) {
        ...
        if (this.LMB){
            ...
            // If LMB, move the camera
            const origo = this.screenToSpace(0, 0);
            const c = this.screenToSpace(event.movementX, event.movementY);

            this.offsetX += (origo.r - c.r) * this.dragSense;
            this.offsetY += (origo.i - c.i) * this.dragSense;
            
            this.render();
        }
        if(this.RMB){

            // If RMB, change zR and zI
            const c = this.screenToSpace(event.clientX, event.clientY);
            this.zR = c.r
            this.zI = c.i
    ...
    */
    static getRenderSource(iterations, zR, zI, gradient, period, numColors, colors, inverse = false) {
        ...
        let inverting = `
            ...
            void main() {
                ...
                float zR = ` + zR.toPrecision(16) + `;
                float zI = ` + zI.toPrecision(16) + `;
                ` + inverting + `
